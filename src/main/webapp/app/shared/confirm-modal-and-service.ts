 ...
 */
@Component({
    selector: 'jhi-confirm-modal-component',
    template: `<div class="modal-header">
    <h4 class="modal-title"><label class="form-control-label" jhiTranslate="{{options.title}}"></label></h4>
    <button type="button" class="close" aria-label="Close" (click)="no()">
      <span aria-hidden="true">&times;</span>
    </button>
    </div>
    <div class="modal-body">
        <label class="form-control-label" jhiTranslate="{{options.message}}"></label>
    </div>
    <div class="modal-footer">
        <button type="button" class="btn btn-danger" (click)="yes()">Si</button>
 ...
 * ```
 * <template jhiConfirm>
 *   <jhi-confirm-modal-component></jhi-confirm-modal-component>
 * </template>
 * ```
 *
